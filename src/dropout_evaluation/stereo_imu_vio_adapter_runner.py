"""Runner for Phase 4.5-B VIO Phase 3 harness adapter smoke."""
...
from .rgbd_odometry_adapter import RgbdAdapterStatus
...
@dataclass
class StereoImuVioAdapterRunSummary:
    algorithm_id: str
    session_id: str
...
@dataclass
class StereoImuVioAdapterRunResult:
    summary: StereoImuVioAdapterRunSummary
    window_diagnostics: list[dict[str, Any]]
    provenance: list[dict[str, Any]]


def run_stereo_imu_vio_adapter_smoke(
    *,
...
) -> StereoImuVioAdapterRunResult:
    adapter_cfg = adapter_config or StereoImuVioAdapterConfig()
    windows = load_dropout_windows_from_manifest(manifest_path)
    local_trajectory = vio_trajectory_to_local_trajectory(load_vio_trajectory_from_csv(continuous_trajectory_csv))
    ...
    for window in windows:
        result = generate_stereo_imu_vio_candidates(
            window=window,
    ...
    return StereoImuVioAdapterRunResult(
        summary=summary,
...
def write_stereo_imu_vio_adapter_smoke_outputs(
    *,
    output_dir: Path,
    run_result: StereoImuVioAdapterRunResult,
) -> None:
    ...
    (output_dir / "README.txt").write_text(
        "Phase 4.5-B VIO adapter smoke output (not an official benchmark artifact).\n",
        encoding="utf-8",
    )


def run_stereo_imu_vio_adapter_smoke_from_paths(
    *,
    config_path: Path,
    manifest_path: Path,
    ...
    result = run_stereo_imu_vio_adapter_smoke(
        config=config,
    ...
    if output_dir is not None:
        write_stereo_imu_vio_adapter_smoke_outputs(output_dir=output_dir, run_result=result)
    return result
